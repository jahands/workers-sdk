...
function updatePackageVersion(packagePath, bumpType = "patch") {
	...
	console.log(
		`✅ Updated ${pkg.name} from ${major}.${minor}.${patch} to ${newVersion}`
	);
	return newVersion;
}
...
function updateWranglerDependency(opencodeVersion) {
	console.log(
		`🔗 Updating wrangler dependency to @jahands/opencode-cf@${opencodeVersion}...`
	);
	...
	if (pkg.dependencies && pkg.dependencies["@jahands/opencode-cf"]) {
		...
		console.log(
			`✅ Updated wrangler dependency to @jahands/opencode-cf@^${opencodeVersion}`
		);
	} else {
		console.log(
			`⚠️  No @jahands/opencode-cf dependency found in wrangler package.json`
		);
	}
}
...
function main() {
	...
	const opencodeVersion = updatePackageVersion(
