...
if [ "$MODE" = "workspace" ]; then
    ...
    if command -v jq >/dev/null 2>&1; then
        jq '.dependencies."@jahands/opencode-cf" = "workspace:*"' package.json >package.json.tmp && mv package.json.tmp package.json
        echo "Updated Wrangler to use workspace:* for @jahands/opencode-cf"
    else
        echo "jq not found. Please manually update packages/wrangler/package.json:"
        echo '  "@jahands/opencode-cf": "workspace:*"'
    fi

    cd ../..
    pnpm install
    echo "✅ Switched to workspace packages. Run 'pnpm turbo build --filter=@jahands/opencode-cf' to build locally."
    ...
    if command -v jq >/dev/null 2>&1; then
        jq ".dependencies.\"@jahands/opencode-cf\" = \"@jahands/opencode-cf@pr-$PR_NUMBER\"" package.json >package.json.tmp && mv package.json.tmp package.json
        echo "Updated Wrangler to use @jahands/opencode-cf@pr-$PR_NUMBER"
    else
        echo "jq not found. Please manually update packages/wrangler/package.json:"
        echo "  \"@jahands/opencode-cf\": \"@jahands/opencode-cf@pr-$PR_NUMBER\""
    fi
